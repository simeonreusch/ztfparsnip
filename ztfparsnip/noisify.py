...
def get_astropy_table(df, headervals, remove_poorconditions = True, phase_lim = True):
        ...
        if phase_lim:
            if headervals['bts_class'] in ['SN IIn','SN IIn?','SLSN-I', 'SLSN-I.5', 'SLSN-I?', 'SLSN-II', 'TDE', 'Baratheon']:
                mask_phase = ((jd - float(headervals['bts_peak_jd'])) < 200.) & ((jd - float(headervals['bts_peak_jd'])) > -50.)
            else:
                mask_phase = ((jd - float(headervals['bts_peak_jd'])) < 50.) & ((jd - float(headervals['bts_peak_jd'])) > -30.)
            jd = jd[mask_phase]
        ...
        phot_tab['sigmapsf'] = np.abs((2.5 * np.log(10)) * (phot_tab['fluxerr'] / phot_tab['flux']))
        phot_tab['zp'] = 25
        phot_tab['zpsys'] = 'ab'
...
def get_noisified_data(lc_table, delta_z, n_sim):
        ...
        for new_z in z_selected:
            ...
            scale = cosmo.luminosity_distance(truez)**2/cosmo.luminosity_distance(new_z)**2
            ...
            if len(mag_new) > 0:
                phot = {'jd': jd_new, 'flux': flux_obs, 'fluxerr': fluxerr_obs, 'magpsf': mag_new,
                    'sigmapsf': magerr_new, 'band': band_new, 'zp': zp_new, 'zpsys': zpsys_new}
                new_lc = Table(phot, names=('jd', 'flux', 'fluxerr', 'magpsf', 'sigmapsf', 'band', 'zp', 'zpsys'),
                       meta = this_lc.meta)
                new_lc.meta['z'] = new_z
...
def noisify_lcs(table, headervals):
    ...
    if delta_m_list != None:
        for i in range(len(new_table_list)):
            new_table_list[i]['magpsf'] = new_table_list[i]['magpsf'].data + delta_m_list[i]
    ...
    for new_table in new_table_list:
        ...
        sig_noise_df = pd.DataFrame(data = {'SN': np.abs(np.array(new_table['flux']/new_table['fluxerr']))})
        count_sn = sig_noise_df[sig_noise_df['SN'] > SN_threshold].count()
        if (new_table['flux'][peak_idx]/new_table['fluxerr'][peak_idx]) > SN_threshold:
